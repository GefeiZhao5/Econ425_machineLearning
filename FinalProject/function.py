...
def admission_decision(Y, alpha=0.25):
    '''
    Y : Probablity of being admitted
    alpha : Admission ratio
    Returns : Admission decision sequence(0 or 1)
    '''
    decision = np.ones((len(Y), 1))
    threshold = np.quantile(Y, 1-alpha)
